...
RunAndSummarize <- function(phy, data, run.type, ...) {
  ...
  par.score<-parsimony(phy, dat, method="fitch")/2 # divided by 2 to deal with duplicate columns
  result.overall <- data.frame(rate01=rate01, rate10=rate10, count0=count0, count1=count1, ntip=Ntip(phy), depth=max(ape::branching.times(phy)), total.length = sum(phy$edge.length), label=run.type, parsimony.score = par.score, stringsAsFactors=FALSE)
...
for (i in sequence(100)) {
  ...
  save(list=ls(), file="SimulationResultsPaired_WithParsimonyScore5PercentTrue.rda")
  try(system("git add SimulationResultsPaired_WithParsimonyScore5PerentTrue.rda"))

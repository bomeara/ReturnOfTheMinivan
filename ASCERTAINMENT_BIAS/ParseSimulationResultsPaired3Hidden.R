rm(list=ls())
library(corHMM)
load("SimulationResultsPaired3Hidden.rda")
library(viridis)
library(car)
library(expm)

equilibrium <- expm(rates*100000)[1,]


truerate01 <- rates['(0,R1)','(1,R1)']*equilibrium['(0,R1)'] + rates['(0,R2)','(1,R2)']*equilibrium['(0,R2)'] + rates['(0,R3)','(1,R3)']*equilibrium['(0,R3)']
truerate10 <- rates['(1,R1)','(0,R1)']*equilibrium['(1,R1)'] + rates['(1,R2)','(0,R2)']*equilibrium['(1,R2)'] + rates['(1,R3)','(0,R3)']*equilibrium['(1,R3)']
...
save(correlations, paired.tests, file="OrganizedSummaryOfPairedSims3Hidden.rda")
...
for (dataset.index in sequence(length(datasets))) {
    pdf(file=paste0("Figure_", names(datasets)[dataset.index], "_pairedtests_3Hidden.pdf"), width=10, height=10)
    par(mfcol=c(2,2))
    #par(mar=c(9,4,4,2)+.1)
    ...
    dev.off()
    Sys.sleep(2)
    #system(paste0("open Figure_", names(datasets)[dataset.index], "_pairedtests3hidden.pdf"))
}

all.clade.names <- unique(pairs.df$empirical.clade.name)
for (clade.index in sequence(length(all.clade.names))) {
  pdf(file=paste0("Figure_", all.clade.names[clade.index], "_pairedtests_3Hidden.pdf"), width=10, height=10)
  par(mfcol=c(2,2))

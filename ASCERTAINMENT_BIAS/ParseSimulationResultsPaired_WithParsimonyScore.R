rm(list=ls())
library(corHMM)
load("SimulationResultsPaired_WithParsimonyScore.rda")
library(viridis)
library(car)
...
#Do dataset with everything included
PlotTraitVersusParsimonyScore <- function(dataset.everything, trait, trait.name, true.val=NA, log="", ...) {
    random.medians <- c()
    random.sds <- c()
    random.counts <- c()
    empirical.medians <- c()
    empirical.sds <- c()
    empirical.counts <- c()
    parsimony.scores <- sequence(max(dataset.everything$empirical.parsimony.score))
    for (parsimony.score in sequence(max(dataset.everything$empirical.parsimony.score))) {
        empirical.trait <- dataset.everything[which(dataset.everything$empirical.parsimony.score>=parsimony.score), which(colnames(dataset.everything)==paste0("empirical.", trait))]
        random.trait <- dataset.everything[which(dataset.everything$random.parsimony.score>=parsimony.score), which(colnames(dataset.everything)==paste0("random.", trait))]
        random.counts[parsimony.score] <- length(random.trait)
        empirical.counts[parsimony.score] <- length(empirical.trait)
        random.medians[parsimony.score] <- median(random.trait, na.rm=TRUE)
        empirical.medians[parsimony.score] <- median(empirical.trait, na.rm=TRUE)
        random.sds[parsimony.score] <- sd(random.trait, na.rm=TRUE)
        empirical.sds[parsimony.score] <- sd(empirical.trait, na.rm=TRUE)
    }
    par(mfcol=c(1,3))
    plot(x=range(parsimony.scores), y=range(c(empirical.medians,random.medians), na.rm=TRUE), type="n", xlab="Minimum parsimony score for trait", ylab=paste0("Median ",trait.name), bty="n", log=log)
    lines(parsimony.scores, empirical.medians, col="blue")
    lines(parsimony.scores, random.medians, col="red")
    if(!is.na(true.val)) {
        abline(h=true.val, lty="dotted")   
    }
    plot(x=range(parsimony.scores), y=range(c(empirical.sds,random.sds), na.rm=TRUE), type="n", xlab="Minimum parsimony score for trait", ylab=paste0("SD ",trait.name), bty="n", log="y")
    lines(parsimony.scores, empirical.sds, col="blue")
    lines(parsimony.scores, random.sds, col="red")
    
    plot(x=range(parsimony.scores), y=range(c(random.counts/nrow(dataset.everything), empirical.counts/nrow(dataset.everything)), na.rm=TRUE), xlab="Minimum parsimony score for trait", type="n", ylab="Fraction of simulations qualifying", bty="n")
    lines(parsimony.scores, empirical.counts/nrow(dataset.everything), col="blue")
    lines(parsimony.scores, random.counts/nrow(dataset.everything), col="red")
    
}


#Do dataset with everything included
PlotTraitVersusParsimonyScoreNotTooMuchVariation <- function(dataset.everything, trait, trait.name, true.val=NA, log="", max.pscore.over.ntax=0.1, ...) {
    random.medians <- c()
    random.sds <- c()
    random.counts <- c()
    empirical.medians <- c()
    ...
    parsimony.scores <- sequence(max(dataset.everything$empirical.parsimony.score))
    for (parsimony.score in sequence(max(dataset.everything$empirical.parsimony.score))) {
        empirical.trait <- dataset.everything[which(dataset.everything$empirical.parsimony.score>=parsimony.score & dataset.everything$empirical.parsimony.score<=max.pscore.over.ntax*dataset.everything$empirical.ntip), which(colnames(dataset.everything)==paste0("empirical.", trait))]
        random.trait <- dataset.everything[which(dataset.everything$random.parsimony.score>=parsimony.score & dataset.everything$random.parsimony.score<=max.pscore.over.ntax*dataset.everything$random.ntip), which(colnames(dataset.everything)==paste0("random.", trait))]
        random.counts[parsimony.score] <- length(random.trait)
        empirical.counts[parsimony.score] <- length(empirical.trait)
        random.medians[parsimony.score] <- median(random.trait, na.rm=TRUE)
        empirical.medians[parsimony.score] <- median(empirical.trait, na.rm=TRUE)
...
#Do dataset with everything included
PlotDeviationVersusParsimonyScore <- function(dataset.everything, trait, trait.name, true.val=NA, ...) {
    random.medians <- c()
    random.sds <- c()
    random.counts <- c()
    empirical.medians <- c()
    ...
    for (parsimony.score in sequence(max(dataset.everything$empirical.parsimony.score))) {
        ...
        random.sds[parsimony.score] <- sd(random.trait, na.rm=TRUE)
        empirical.sds[parsimony.score] <- sd(empirical.trait, na.rm=TRUE)
        w.result <- wilcox.test(empirical.trait, random.trait, mu=0, conf.int=TRUE)
        p.values[parsimony.score] <- w.result$p.value
        estimates[parsimony.score] <- w.result$estimate
        conf.min[parsimony.score] <- w.result$conf.int[1]
        conf.max[parsimony.score] <-  w.result$conf.int[2]
    }
    par(mfcol=c(1,1))
    plot(x=range(parsimony.scores), y=range(c(conf.min, conf.max)), type="n", xlab="Minimum parsimony score for trait", ylab=paste0("Named - random in trait ", trait.name), bty="n")
    lines(parsimony.scores, estimates, lwd=3)
    lines(parsimony.scores, conf.min)
    lines(parsimony.scores, conf.max)
    abline(h=0, col="gray")
    for(i in sequence(length(parsimony.scores))) {
        if(p.values[i] < 0.05) {
            points(x=parsimony.scores[i], y=estimates[i], pch=19, col="red")   
        }
    }
...
save(correlations, paired.tests, file="OrganizedSummaryOfPairedSims_WithParsimony.rda")
...
logs <- c("y", "", "y", "y")
traits <- c("average.rate", "tropic.to.temperate.fraction", "abs.average.rate.deviation", "abs.tropic.to.temperate.fraction.deviation")
true.vals <- c(0.5*(truerate01+truerate10), truerate10/(truerate01+truerate10), NA, NA)

for(trait.index in sequence(length(trait.names.pretty))) {
    pdf(file=paste0("FigureIndividualTrait_", traits[trait.index], "_pairedtests_WithParsimony.pdf"), width=10, height=5)
    PlotTraitVersusParsimonyScore(datasets[[1]], traits[trait.index], trait.names.pretty[trait.index], true.val=true.vals[trait.index], log=logs[trait.index])
    dev.off()
    pdf(file=paste0("FigureIndividualTraitDeviation_", traits[trait.index], "_pairedtests_WithParsimony.pdf"), width=10, height=5)
    PlotDeviationVersusParsimonyScore(datasets[[1]], traits[trait.index], trait.names.pretty[trait.index])
    dev.off()
    pdf(file=paste0("FigureIndividualTraitDeviationNotTooMuch_", traits[trait.index], "_pairedtests_WithParsimony.pdf"), width=10, height=5)
    PlotTraitVersusParsimonyScoreNotTooMuchVariation(datasets[[1]], traits[trait.index], trait.names.pretty[trait.index])
    dev.off()
    
}

for (dataset.index in sequence(length(datasets))) {
    pdf(file=paste0("Figure_", names(datasets)[dataset.index], "_pairedtests_WithParsimony.pdf"), width=10, height=10)
    par(mfcol=c(2,2))
    #par(mar=c(9,4,4,2)+.1)
...
for (clade.index in sequence(length(all.clade.names))) {
  pdf(file=paste0("Figure_", all.clade.names[clade.index], "_pairedtests_WithParsimony.pdf"), width=10, height=10)
  par(mfcol=c(2,2))
...
pdf(file="RatesPaired_WithParsimony.pdf", width=10, height=10)
...
pdf(file="AverageRateVsNtaxPaired_WithParsimony.pdf", width=10, height=10)
...
pdf(file="RateRatioVsNtaxPaired_WithParsimony.pdf", width=10, height=10)
...
write.csv(all.sims.df, file="AllSimRates_WithParsimony.csv")

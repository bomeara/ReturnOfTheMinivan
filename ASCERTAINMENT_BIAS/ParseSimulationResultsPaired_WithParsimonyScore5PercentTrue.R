...
PlotPairs <- function(pairs.df, trait, trait.name, main, ...) {
    ...
    x1 <- pairs.df[, which(colnames(pairs.df)==paste0("empirical.", trait))]
    x2 <- pairs.df[, which(colnames(pairs.df)==paste0("random.", trait))]
    bad <- unique(c(which(is.na(x1)), which(is.na(x2))))
    if(length(bad)>0) {
        x1 <- x1[-bad]
        x2 <- x2[-bad]
    }
...
for(trait.index in sequence(length(trait.names.pretty))) {
    pdf(file=paste0("FigureIndividualTrait_", traits[trait.index], "_pairedtests_WithParsimony5Percent.pdf"), width=10, height=5)
    ...
    dev.off()
    pdf(file=paste0("FigureIndividualTraitDeviation_", traits[trait.index], "_pairedtests_WithParsimony5Percent.pdf"), width=10, height=5)
...
for (clade.index in sequence(length(all.clade.names))) {
  pdf(file=paste0("Figure_", all.clade.names[clade.index], "_pairedtests_WithParsimony5Percent.pdf"), width=10, height=10)
  par(mfcol=c(2,2))
...
pdf(file="RatesPaired_WithParsimony5Percent.pdf", width=10, height=10)
plot(x=range(all.sims.df$average.rate.median, na.rm=TRUE), y=range(all.sims.df$tropic.to.temperate.fraction.median, na.rm=TRUE), bty="n", type="n", xlab="Average transition rate", ylab="Tropical to temperate rate fraction", log="x")
...
pdf(file="AverageRateVsNtaxPaired_WithParsimony5Percent.pdf", width=10, height=10)
plot(x=range(all.sims.df$ntip.median, na.rm=TRUE), y=range(all.sims.df$average.rate.median, na.rm=TRUE), bty="n", xlab="Number of taxa", ylab="Average transition rate", type="n", log="xy")
...
pdf(file="RateRatioVsNtaxPaired_WithParsimony5Percent.pdf", width=10, height=10)
plot(x=range(all.sims.df$ntip.median, na.rm=TRUE), y=range(all.sims.df$tropic.to.temperate.fraction.median, na.rm=TRUE), bty="n", xlab="Number of taxa", ylab="Tropical to temperate rate fraction", type="n", log="x")
abline(h=truerate10/(truerate01+truerate10), col="red", lty="dotted")
...
write.csv(all.sims.df, file="AllSimRates_WithParsimony5Percent.csv")
